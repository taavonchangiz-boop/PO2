    ...
    for (var i = 0; i < parts.length; i++) {
        ...
        if (i === 0) {
            ...
            if (adminMatch) {
                ...
                title.style.cssText = 'font-size:0.8rem; color:#F5BC82; font-weight:900; margin-bottom:0.4rem;';
                title.textContent = '👑 پیام مدیر سیستم (' + adminMatch[1] + '):';
                var date = document.createElement('div');
                date.style.cssText = 'font-size:0.7rem; color:#7A7062; margin-bottom:0.5rem;';
                date.textContent = '📅 ' + adminMatch[2];
                var content = document.createElement('div');
                content.style.whiteSpace = 'pre-wrap';
                content.textContent = adminMatch[3];
                bubble.append(title, date, content);
            } else {
                var userTitle = document.createElement('div');
                userTitle.style.cssText = 'font-size:0.75rem; color:#E9C77E; font-weight:bold; margin-bottom:0.4rem;';
                userTitle.textContent = '👤 پیام کاربر (' + (t.user_name || "کاربر") + '):';
                var userContent = document.createElement('div');
                userContent.style.whiteSpace = 'pre-wrap';
                userContent.textContent = text;
                bubble.append(userTitle, userContent);
            }
        } else {
            ...
            if (supportMatch || userReplyMatch) {
                var safeHeader = document.createElement('div');
                safeHeader.style.cssText = 'font-size:0.8rem; font-weight:900; margin-bottom:0.4rem;';
                safeHeader.textContent = supportMatch ? '👑 پاسخ کارشناس پشتیبانی پُست‌یار:' : '👤 پاسخ شما:';
                var safeDate = document.createElement('div');
                safeDate.style.cssText = 'font-size:0.7rem; color:#7A7062; margin-bottom:0.5rem;';
                safeDate.textContent = '📅 ' + (supportMatch ? supportMatch[1] : userReplyMatch[1]);
                var safeBody = document.createElement('div');
                safeBody.style.whiteSpace = 'pre-wrap';
                safeBody.textContent = bodyText;
                bubble.append(safeHeader, safeDate, safeBody);
            } else {
                var safeBodyOnly = document.createElement('div');
                safeBodyOnly.style.whiteSpace = 'pre-wrap';
                safeBodyOnly.textContent = bodyText;
                bubble.append(safeBodyOnly);
            }
        }
